...
//	}
	...
	public static void setNumToEmail(HashMap<String,String> h)
	{
		numToEmail = h;
	}
	public static void setEmailToNum(HashMap<String,String> h)
	{
		emailToNum = h;
	}
	...
	public static String map(String devi)
	{
		if(devi.matches(CommonRegex.PHONE_NUMBER) && !devi.matches(CommonRegex.EMAIL_ADDRESS))
		{
			return getEmail(devi);
		}
		
		if(!devi.matches(CommonRegex.PHONE_NUMBER) && devi.matches(CommonRegex.EMAIL_ADDRESS))
		{
			return getNumber(devi);
		}
	...
	public static void setConfig(JsonNode map)
	{
		...
		{
			PittsburgWeb.LOG.debug("Falure Saving Map Configurations",e);
		}
	}
}
